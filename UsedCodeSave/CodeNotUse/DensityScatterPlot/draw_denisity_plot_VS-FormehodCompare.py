...
# 第一个文件（y 轴数据）
fusion_output_files = [
     "/DeepLearning/mnt/shixiansheng/data_fusion/output/2011_FtA_DFT_filteredForMap_ROWCOL.csv",
]

# 第二个文件（x 轴数据）
x_axis_file ="/DeepLearning/mnt/shixiansheng/data_fusion/output/2011_FtA_DFT_filteredForMap_ROWCOL.csv" # 替换为实际路径

# -------------------- 创建文件夹 --------------------
output_dir = '/DeepLearning/mnt/shixiansheng/data_fusion/output/scatter_plots_FtA_DFTVsFtA_DFT_InUSA'
...
def plot_density_scatter(dataframe_x, dataframe_y, x_column, y_column, period_column, output_dir, period_value, file_name):
    ...
    # 只提取季节和年份部分
    period_search = re.search(r"(DJF|MAM|AAJ|SON|Apr-Sep|Annual|top-10|98th)", period_value)
    if period_search:
    ...
    # 设置标题和标签
    ax.set_xlabel(f'{x_label} ({x_column})', fontsize=13)  # x 轴标签
    ax.set_ylabel(f'{y_label} ({y_column})', fontsize=13)  # y 轴标签
...
def process_file(fusion_output_file):
    ...
    # 新增：过滤掉值为 2 的点,对于draw_denisity_plot来说，这边去除后不影响后续MAE等的统计计算,与data_fusion_plot_MissingDataCal联动
    df_data_x = df_data_x[(df_data_x["vna_ozone"] != 2) & (df_data_x["evna_ozone"] != 2)&(df_data_x["model"] != 2)]
    df_data_y = df_data_y[(df_data_y["vna_ozone"] != 2) & (df_data_y["evna_ozone"] != 2)&(df_data_x["model"] != 2)]
