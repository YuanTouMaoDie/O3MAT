...
@timer_decorator
def start_daily_data_fusion(model_file, monitor_file, region_table_file, file_path, monitor_pollutant="pm25",
    ...
    with tqdm(dates) as pbar:
        for date in pbar:
            ...
            df_fusion["avna_pm25"] = ds_daily_model[model_pollutant][0].values - df_fusion["vna_bias"]
            reshaped_vna_r_n = df_prediction["vna_r_n"].values.reshape(ds_daily_model[model_pollutant][0].shape)
            df_fusion["evna_pm25"] = (("ROW", "COL"), ds_daily_model[model_pollutant][0].values * reshaped_vna_r_n)
...
def save_daily_data_fusion_to_metrics(df_data, save_path, project_name):
    '''
    This function converts the daily data fusion results to O3-related metrics (e.g., 98th percentile of MDA8 pm25 concentration, average of top-10 MDA8 pm25 days, annual average of MDA8 pm25 concentration) files.
...
if __name__ == "__main__":
    ...
    monitor_file = r"/DeepLearning/mnt/shixiansheng/data_fusion/3_OTHER/DFT_output/PM25ForFractions2017_WithoutZero.csv"
    ...
    daily_output_path = os.path.join(save_path, "20170102_PM25_PythonFtA_CONUS.csv")
