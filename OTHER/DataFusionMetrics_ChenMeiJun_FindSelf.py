...
def start_daily_data_fusion(model_file, monitor_file, save_path, variables):
    ...
    for variable in variables:
        ...
        for season in seasons:
            ...
            # 将模型数据转换为xarray.Dataset
            ds_daily_model = df_model.set_index(['ROW', 'COL']).to_xarray()

            # 查找监测点所在的模型网格点
            df_daily_obs["mod"] = ds_daily_model[model_pollutant].sel(
            ...
            df_daily_obs["bias"] = df_daily_obs["mod"] - df_daily_obs[monitor_pollutant]
            df_daily_obs["r_n"] = df_daily_obs[monitor_pollutant] / df_daily_obs["mod"]
        ...
        final_result = final_result[['ROW', 'COL', 'Period', 'Variable', 'model', 'vna_ozone', 'avna_ozone', 'evna_ozone']]
