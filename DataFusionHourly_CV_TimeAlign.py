...
@timer_decorator
def start_hourly_data_fusion(model_files, monitor_file, cross_validation_file, file_path, monitor_pollutant="ozone",
    ...
    df_ds_ozone['Date'] = pd.to_datetime(df_ds_ozone['Date'])
    ...
    with tqdm(dates) as pbar:
        for date in pbar:
            ...
            df_hourly_obs = df_obs_grouped[df_obs_grouped["dateon"] == date].copy()

            # 修改部分：获取当天的所有交叉验证数据
            df_daily_cv = df_cv[df_cv["Date"].dt.date == date.date()].copy()

            # 提取和交叉验证表站点一样的数据
            df_hourly_obs = df_hourly_obs[df_hourly_obs["site_id"].isin(df_daily_cv["Site"])]
            ...
            df_hourly_obs["x"], df_hourly_obs["y"] = proj(df_hourly_obs["Lon"], df_hourly_obs["Lat"])

            # 记录匹配的模型网格点信息
            matched = ds_hourly_model[model_pollutant][0].sel(
                ROW=df_hourly_obs["y"].to_xarray(),
                COL=df_hourly_obs["x"].to_xarray(),
                method="nearest",
                drop=True
            )
            df_hourly_obs["mod"] = matched
            df_hourly_obs["ROW"] = matched.ROW.values
            df_hourly_obs["COL"] = matched.COL.values
            ...
            for cv_group in cv_groups:
                ...
                df_train = df_hourly_obs[df_hourly_obs["site_id"].isin(train_sites)].copy()
                df_test = df_hourly_obs[df_hourly_obs["site_id"].isin(test_sites)].copy()

                # 检查训练集和测试集是否为空
                if df_train.empty or df_test.empty:
                    print(f"警告：日期 {date} 的训练集或测试集为空，跳过此 CV 组处理。")
                    continue
                ...
                njobs = min(len(df_test), 10)
                zdf = nn.predict(test_data)
                ...
                # 给 ROW 和 COL 加上 0.5
                df_test.loc[:, "ROW"] = (df_test["site_id"].map(df_hourly_obs.set_index("site_id")["ROW"]) + 0.5).astype(int)
                df_test.loc[:, "COL"] = (df_test["site_id"].map(df_hourly_obs.set_index("site_id")["COL"]) + 0.5).astype(int)

                df_test.loc[:, "CVgroup"] = cv_group
    ...
    if df_all_hourly_prediction is not None:
        # 合并 ds_ozone 数据
        df_all_hourly_prediction = pd.merge(df_all_hourly_prediction, df_ds_ozone, left_on=["dateon", "site_id"], right_on=["Date", "Site"], how="left")

        # 调整 Site 顺序
        site_order = df_ds_ozone["Site"].unique()
        df_all_hourly_prediction["site_id"] = pd.Categorical(df_all_hourly_prediction["site_id"], categories=site_order, ordered=True)
        df_all_hourly_prediction = df_all_hourly_prediction.sort_values(by=["dateon", "site_id"])

        # 将 dateon 转换为小时格式
        df_all_hourly_prediction['dateon'] = df_all_hourly_prediction['dateon'].dt.strftime('%Y-%m-%d %H:00')

        df_all_hourly_prediction = df_all_hourly_prediction[["dateon", "site_id", "Lat", "Lon", "O3", "CVgroup", "model", "vna_ozone", "evna_ozone", "avna_ozone", "ds_ozone", "ROW", "COL"]]
