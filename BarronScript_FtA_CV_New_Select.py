...
def start_daily_data_fusion(model_file, monitor_file, file_path, monitor_pollutant="ozone", model_pollutant="O3",
    ...
    # 根据交叉验证类型处理数据
    if cv_type == 'kfold':
        if 'CV_Name' in df_obs.columns:
            # 使用自定义的 K 折交叉验证
            fold_names = sorted(df_obs['CV_Name'].dropna().unique())
            for date in dates:
                print(f"Processing date {date}")
                df_daily_obs = df_obs_grouped[df_obs_grouped["Date"] == date].copy()
                daily_prediction_dfs = []
                for fold, fold_name in enumerate(fold_names, 1):
                    fold_start_time = time.time()
                    print(f"Processing fold {fold} with fold name {fold_name} for {date}")
                    df_train = df_obs[df_obs['CV_Name'] != fold_name].copy()
                    df_test = df_obs[df_obs['CV_Name'] == fold_name].copy()
                    df_result, results = process_data(df_train, df_test, ds_model, proj, nn, model_pollutant, monitor_pollutant, fold, date)
        ...
        print("Processing custom cross-validation")
        df_train = df_obs[df_obs["Site"].isin(train_sites)].copy()
        df_test = df_obs[df_obs["Site"].isin(test_sites)].copy()
        for date in dates:
            print(f"Processing date {date}")
            df_result, results = process_data(df_train, df_test, ds_model, proj, nn, model_pollutant, monitor_pollutant, fold=0, date=date)
            all_daily_prediction_dfs.append(df_result)
            all_results.extend(results)
            # 每天的数据处理完后保存到文件
            daily_file_path = file_path.replace('.csv', f'_{date}.csv')
            df_result.to_csv(daily_file_path, index=False)
            print(f"Data Fusion for {date} is done, the results are saved to {daily_file_path}")
        custom_end_time = time.time()
        custom_duration = custom_end_time - custom_start_time
        print(f"Custom cross-validation took {custom_duration:.2f} seconds")
    else:
        for date in dates:
            print(f"Data Fusion for {date}...")
            start_time = time.time()

            df_daily_obs = df_obs_grouped[df_obs_grouped["Date"] == date].copy()
            df_fusion, _ = process_daily_data(df_daily_obs, ds_model, proj, nn, model_pollutant, monitor_pollutant, date, 0)
            all_daily_prediction_dfs.append(df_fusion)

            # 每天的数据处理完后保存到文件
            daily_file_path = file_path.replace('.csv', f'_{date}.csv')
            df_fusion.to_csv(daily_file_path, index=False)
            print(f"Data Fusion for {date} is done, the results are saved to {daily_file_path}")
...
def process_data(df_train, df_test, ds_model, proj, nn, model_pollutant, monitor_pollutant, fold, date):
    """
    处理训练集和测试集数据的辅助函数。
...
if __name__ == "__main__":
    ...
    # 移除重复的散点图绘制调用
    # results_file_path = os.path.join(os.path.dirname(output_file), os.path.basename(output_file).replace('.csv', '_CV_results.csv'))
    # plot_scatter_from_file(results_file_path, "Conc")
