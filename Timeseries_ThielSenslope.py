...
for period in tqdm(periods):
    ...
    # 对 harvard_ml 变量单独处理，只计算 2002-2016 年的数据
    harvard_ml_data = period_data[(period_data['Year'] >= 2002) & (period_data['Year'] <= 2016)]
    result_df = period_data.groupby(['ROW', 'COL']).apply(
        lambda group: pd.Series({var: theilslopes(group[var], group['Year'])[0] for var in variables if var != 'harvard_ml'})
    ).reset_index()
    # 计算 harvard_ml 的泰尔 - 森斜率并合并到结果中
    if not harvard_ml_data.empty:
        harvard_ml_result = harvard_ml_data.groupby(['ROW', 'COL']).apply(
            lambda group: pd.Series({'harvard_ml': theilslopes(group['harvard_ml'], group['Year'])[0]})
        ).reset_index()
        result_df = pd.merge(result_df, harvard_ml_result, on=['ROW', 'COL'], how='outer')
