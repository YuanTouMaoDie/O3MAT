...
def plot_us_map(
    ...
    variables_to_plot=['vna_ozone', 'evna_ozone', 'avna_ozone'],
    ...
    @param {list} variables_to_plot: list of variables to plot
    ...
    for key_period in tqdm(key_periods):
        ...
        for variable in variables_to_plot:
            if variable not in df_x_period.columns or variable not in df_y_period.columns:
                print(f"Variable {variable} not found in data files. Skipping...")
                continue

            # Reshape data to match the grid
            grid_variable_x = df_x_period[variable].values.reshape(longitudes.shape)
            grid_variable_y = df_y_period[variable].values.reshape(longitudes.shape)

            # Calculate differences (y - x)
            grid_variable_diff = grid_variable_y - grid_variable_x

            # Get settings for the current variable
            settings = variable_settings.get(variable, {})
            ...
            fig = show_maps(
                ...
                delta_map_settings=settings.get('delta_map_settings', {
                    "cmap": cmap_delta,
                    "value_range": (-3, 3),  # Let the function auto-scale
            ...
            if save_path is not None:
                save_file = os.path.join(save_path, f"{year}_{key_period}_{variable}_diff_data_fusion.png")
                fig.savefig(save_file, dpi=300, bbox_inches="tight")
                print(f"The data fusion plot for {year} {key_period} {variable} is saved to {save_file}")
...
if __name__ == "__main__":
    ...
    fusion_output_file_x = r"/DeepLearning/mnt/shixiansheng/data_fusion/output/Harvard_model_2011_dailyIndex.csv"
    ...
    fusion_output_file_y = r"/DeepLearning/mnt/shixiansheng/data_fusion/output/EQUATES_model_2011_dailyIndex.csv"
    ...
    # 指定要绘制的变量
    variables_to_plot = ['model','vna_ozone', 'evna_ozone', 'avna_ozone']
