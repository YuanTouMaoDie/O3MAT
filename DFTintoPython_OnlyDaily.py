...
def merge_data_file(model_file, year, start_date=None, end_date=None):
    ...
    df_model = pd.read_csv(model_file, header=1)
    ...
    for day in day_data.unique():  # 逐个日期处理
        ...
        # 获取VNA和EVNA数据
        vna_data = df_model.loc[df_model['day'] == day, 'O3_VNA'].dropna().reset_index(drop=True)
...
# 保存合并后的数据
output_file = '/DeepLearning/mnt/shixiansheng/data_fusion/output/DFT_VNA_2011_Year_daily.csv'
