...
def process_row(row, is_valid, tz_file='output/Region/tz.csv'):
    if is_valid:
        lon = float(row[2])
        lat = float(row[3])
        # 不加负号，就为正
...
def process_data(input_file, output_file, is_file):
    global first_time
    first_time = True
    global lines
    lines = []
    ...
    # 读取 Is 文件
    is_data = {}
    with open(is_file, 'r') as is_f:
        is_reader = csv.reader(is_f)
        next(is_reader)  # 跳过标题行
        for row in is_reader:
            row_num = int(row[0])
            col_num = int(row[1])
            is_valid = row[2].strip() == '1'
            is_data[(row_num, col_num)] = is_valid
    ...
    with open(input_file, 'r') as infile:
        ...
        for row in reader:
            row_num = int(row[0])
            col_num = int(row[1])
            is_valid = is_data.get((row_num, col_num), False)
            rows.append(row)
        ...
        # 使用 pool.map 保证顺序
        results = pool.starmap(process_row_partial, zip(rows, valid_flags))
    ...
    for row in new_rows[1:]:  # 跳过标题行
        gmt_offset = row[-1]
        if gmt_offset:
            gmt_offset_count[gmt_offset] = gmt_offset_count.get(gmt_offset, 0) + 1
...
output_file = 'output/Region/ROWCOLRegion_Tz_CONUS.csv'
is_file = 'output/Region/Region_CONUSHarvard.csv'
process_data(input_file, output_file, is_file)