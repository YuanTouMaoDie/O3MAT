...
def calculate_other_metrics(mda8_df, save_path, project_name):
    ...
    for period, (aggregator, group_cols, temp_df) in periods.items():
        ...
        with tqdm(total=total_grids, desc=f"Processing {period} ROW - COL grids") as pbar:
            for (row, col), group in grouped:
                row_col_metrics = {'ROW': row, 'COL': col, 'Period': period}
                for col in ozone_columns:
                    if group.empty:
                        row_col_metrics[col] = np.nan
                    else:
                        # 修改聚合结果提取方式
                        agg_result = group.agg({col: aggregator})
                        if isinstance(agg_result, pd.Series):
                            agg_value = agg_result.values[0]
                        elif isinstance(agg_result, pd.DataFrame):
                            agg_value = agg_result.iloc[0, 0]
                        else:
                            agg_value = agg_result
                        row_col_metrics[col] = agg_value
                all_metrics.append(row_col_metrics)
            pbar.update(1)

    final_df = pd.DataFrame(all_metrics)
    columns_order = ['ROW', 'COL', 'Period', 'vna_ozone', 'evna_ozone', 'avna_ozone', 'model']
