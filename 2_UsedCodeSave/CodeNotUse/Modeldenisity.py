...
def plot_density_scatter(dataframe_x, dataframe_y, x_column, y_column, period_column, output_dir, period_value, file_name):
    ...
    # 记录缺失数据的 id
    nan_indices_x = np.where(np.isnan(x_data))[0]
    nan_indices_y = np.where(np.isnan(y_data))[0]
    # 合并缺失数据的 id
    all_nan_indices = np.unique(np.concatenate((nan_indices_x, nan_indices_y)))

    # 移除缺失数据
    valid_indices = np.setdiff1d(np.arange(len(x_data)), all_nan_indices)
...
def process_file(model):
    ...
    if year_x != year_y:
        ...
        y_label = get_axis_label(fusion_output_file, year=year)
...
model = [
    [
        ...
        "/DeepLearning/mnt/shixiansheng/data_fusion/output/BarronResult_VNAeVNA_2011_FtAIndex.csv"
    ]
]

# 使用 ProcessPoolExecutor 实现并行化
with ProcessPoolExecutor() as executor:
    executor.map(process_file, model)