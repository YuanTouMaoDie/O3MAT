...
def start_hourly_data_fusion(model_files, monitor_file_template, region_table_file, time_region_file, file_path,
    ...
    df_rn_filtered_2011 = df_rn_filtered_2011[
        (df_rn_filtered_2011['localtime'].dt.month >= 7) & (df_rn_filtered_2011['localtime'].dt.month <= 7)]

    # 保存筛选后的数据表
    rn_filtered_file_path = os.path.join(os.path.dirname(file_path), 'rn_filtered_data_Monthly.csv')
    ...
    # 保存平均值数据表
    mean_file_path = os.path.join(os.path.dirname(file_path), 'mean_data_by_site_Monthly.csv')
...
if __name__ == "__main__":
    ...
    # # # 指定日期范围
    start_date = '2011/07/01 00:00'
    end_date = '2011/08/01 08:00'
