...
def start_hourly_data_fusion(model_files, monitor_file_template, region_table_file, time_region_file, file_path,
    ...
    for col in required_columns:
        if col not in df_obs.columns:
            print(f"Error: Column {col} not found in the monitor file.")
            return

    # 根据时区偏移量调整监测时间到UTC，由于gmt_offset为负，所以做减法，同时整体加1小时
    df_obs['utc_dateon'] = df_obs.apply(
        lambda row: row['dateon'] - pd.Timedelta(hours=row['gmt_offset']) + pd.Timedelta(hours=1), axis=1
    )
...
if __name__ == "__main__":
    ...
    # 指定日期范围
    start_date = '2011/08/02 02:00'
    end_date = '2011/08/02 02:00'

    daily_output_path = os.path.join(save_path, "2011_SixDataset_Hourly_Test.csv")
