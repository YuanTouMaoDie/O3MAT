...
def process_daily_data(df_daily_obs, ds_model, proj, nn, model_pollutant, monitor_pollutant, date, fold):
    ...
    all_grid_points = df_prediction[['COL', 'ROW']].values
    zdf = nn.predict(all_grid_points, njobs=njobs)

    df_prediction["vna_ozone"] = zdf[:, 0]
...
@timer_decorator
def start_daily_data_fusion(model_file, monitor_file, file_path,
...
def generate_result_table(model_file, monitor_file, monitor_pollutant="Conc", model_pollutant="O3_MDA8", k=10, date_range=['2011-01-01', '2011-01-01']):
    ds_model = pyrsig.open_ioapi(model_file)
    proj = pyproj.Proj(ds_model.crs_proj4)
...
if __name__ == "__main__":
    ...
    output_file_name = "FourDataset_CV.csv"
    ...
    # 生成结果表
    generate_result_table(model_file, monitor_file)
