...
def start_hourly_data_fusion(model_files, monitor_file, region_table_file, file_path, monitor_pollutant="Ozone",
                            model_pollutant="O3_MDA8", start_date=None, end_date=None):
    """
    ...
    with tqdm(dates) as pbar:
        for date in pbar:
            ...
            df_fusion["Timestamp"] = date.strftime('%Y-%m-%d %H:%M:%S')  # 将日期格式化为包含小时的字符串
...
if __name__ == "__main__":
    ...
    monitor_file = r"/backupdata/data_EPA/EQUATES/2011_Hour_Data/AQS_hourly_data_2011_LatLon.csv"
