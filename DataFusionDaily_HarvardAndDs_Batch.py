...
def extract_harvard_nc_to_dataframe(nc_file, region_df):
    """
    从哈佛 NetCDF 文件中提取臭氧数据，计算指标，并返回 DataFrame。

    参数:
    - nc_file: 哈佛 NetCDF 文件的路径。
    - region_df: 包含 Is 列的区域数据 DataFrame。
    ...
    valid_rows = us_region_df['ROW'].tolist()
    valid_cols = us_region_df['COL'].tolist()
    ...
    for i in tqdm(range(tstep), desc="处理时间步长"):
        ...
        for row in valid_rows:
            for col in valid_cols:
                data['ROW'].append(row)
                data['COL'].append(col)
                data['Ozone'].append(ozone_data[i, 0, row - 1, col - 1].item())
...
def extract_ds_nc_to_dataframe(nc_file, region_df):
    """
    ...
    for i in tqdm(range(tstep)):
        ...
        for row in valid_rows:
            for col in valid_cols:
                data['ROW'].append(row)
                data['COL'].append(col)
                data['Ozone'].append(mda_o3[i, 0, row - 1, col - 1].item())
...
existing_df_path = '/DeepLearning/mnt/shixiansheng/data_fusion/output/2011_Data_WithoutCV/2011_SixDataset_Daily_Metrics.csv'
region_table_file = '/DeepLearning/mnt/shixiansheng/data_fusion/output/Region/Region_CONUSHarvard.csv'

# 读取包含 Is 列的数据表
region_df = pd.read_csv(region_table_file)

# 处理哈佛 ML 数据并合并
harvard_df = extract_harvard_nc_to_dataframe(harvard_nc_file, region_df)
merge_with_existing(existing_df_path, harvard_df)

# 处理 ds_ozone 数据并合并
ds_df = extract_ds_nc_to_dataframe(ds_nc_file, region_df)
